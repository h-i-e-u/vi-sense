...
const AnalyzeLink: React.FC = () => {
  ...
  const [platform, setPlatform] = useState<'youtube' | 'shopee' | 'tiki'>('youtube');
  ...
  const getPlatformColor = (plat: string) => {
    switch (plat) {
      ...
      case 'tiki': return 'text-blue-400';
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            >
              <h1 className="text-3xl font-bold text-white mb-2">Analyze Link</h1>
              <p className="text-white/60">Extract and analyze sentiment from YouTube, Shopee, or Tiki</p>
            </motion.div>
            ...
            <GlassCard className="p-6">
              ...
              <AnimatedButton
                ...
                className="w-full"
              >
                {isAnalyzing ? 'Analyzing...' : `Analyze ${platform.charAt(0).toUpperCase() + platform.slice(1)}`}
              </AnimatedButton>
            </GlassCard>
              ...
              >
                <GlassCard className="p-6">
                  ...
                  <div className="space-y-4">
                    <h4 className="text-lg font-medium text-white">Recent Comments</h4>
                    {result.results?.slice(0, 10).map((sentiment, index) => (
                      <div key={index} className="flex items-start space-x-3 p-4 bg-white/5 rounded-lg">
                        <div className="flex-1">
                          <p className="text-white text-sm line-clamp-3">{sentiment.text}</p>
                        </div>
                        <SentimentBadge sentiment={sentiment.label} confidence={sentiment.confidence} />
                      </div>
                    ))}
                  </div>
                </GlassCard>
            ...
            <LoadingOverlay isLoading={isAnalyzing} message={`Analyzing ${platform} content...`} />
          </div>
        </div>
      </div>
    </div>
