...
class SentimentAnalyzer:
    ...
    def _load_custom_model(self):
        ...
        # Đọc môi trường để quyết định lấy model từ Local hay từ Hugging Face Hub
        env_mode = os.getenv("ENV_MODE", "development").lower()

        if env_mode in ["production", "prod", "staging"]:
            # Tên Repo Model chính của bạn trên Hugging Face
            model_repo = "h-i-e-u/phobert-and-svm-absa"
            # Đường dẫn thư mục bên trong Repo (Thay bằng tên folder chính xác của bạn)
            target_subfolder = "best_phobert_model" 
            
            print(f"📥 Downloading artifacts from {model_repo} [{target_subfolder}]")

            # 1. Tải cấu hình Tokenizer từ subfolder
            self.tokenizer = AutoTokenizer.from_pretrained(
                model_repo, 
                subfolder=target_subfolder
            )

            # 2. Tải file trọng số phobert_sentiment.pth từ subfolder
            from huggingface_hub import hf_hub_download
            weights_path = hf_hub_download(
                repo_id=model_repo, 
                subfolder=target_subfolder,
                filename="phobert_sentiment.pth"
            )
        else:
            # --- Cấu hình trên LOCAL DEVELOPMENT (Giữ nguyên như cũ) ---
            base_dir = Path(__file__).resolve().parent
            model_path = base_dir.parent.parent / "best_phobert_model"
            ...
            weights_path = os.path.join(model_path, "phobert_sentiment.pth")

        # Khởi tạo kiến trúc Class Model của bạn
        self.model = HybridPhoBERTClassifier()

        # Load trọng số đã tải về (dù là từ local hay từ Hub) vào Model
        ...
        self.model.eval()
        print("✅ Custom PhoBERT model loaded successfully!")
