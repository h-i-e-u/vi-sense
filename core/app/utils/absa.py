    ...
    def _ensure_model_loaded(self):
        ...
        with self._model_lock:
            if not self._model_loaded:
                # Kiểm tra môi trường để quyết định nguồn tải model
                env_mode = os.getenv("ENV_MODE", "development").lower()

                if env_mode in ["production", "prod", "staging"]:
                    # --- CẤU HÌNH TRÊN PRODUCTION (HUGGING FACE SPACES) ---
                    model_repo = "h-i-e-u/phobert-and-svm-absa"
                    target_subfolder = "absa_model" 
                    
                    print(f"📥 Downloading ABSA SVM model from Hub: {model_repo} [{target_subfolder}]")
                    
                    from huggingface_hub import hf_hub_download
                    try:
                        # Tự động tải file .pkl từ subfolder của Repo về cache hệ thống
                        resolved_path = hf_hub_download(
                            repo_id=model_repo,
                            subfolder=target_subfolder,
                            filename="absa_svm_model.pkl"
                        )
                    except Exception as e:
                        print(f"❌ Failed to download from subfolder, trying root repo... Error: {e}")
                        # Dự phòng nếu file nằm ngay ngoài thư mục gốc của Repo Model Hub
                        resolved_path = hf_hub_download(
                            repo_id=model_repo,
                            filename="absa_svm_model.pkl"
                        )
                else:
                    # --- CẤU HÌNH TRÊN LOCAL DEVELOPMENT (Quét file local như cũ) ---
                    resolved_path = self._resolve_model_path()
                    print(f"📁 Loading ABSA SVM model from local path: {resolved_path}")

                # Load model từ đường dẫn đã được giải quyết (local hoặc cache hub)
                self.model = joblib.load(resolved_path)
                self._model_loaded = True
                print("✅ ABSA SVM model loaded successfully!")
